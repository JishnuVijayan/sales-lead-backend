...
export class ReportsService {
  ...
  async getPerformanceMetrics(startDate?: Date, endDate?: Date): Promise<PerformanceMetrics> {
    ...
    // Find top performers
    const salesExecutives = userStats.filter(user => user.role === UserRole.ACCOUNT_MANAGER);
  ...
  async getLeadAgingSummary(startDate?: Date, endDate?: Date): Promise<LeadAgingBucket[]> {
    let query = this.leadsRepository.createQueryBuilder('lead');
    ...
    const agingBuckets: LeadAgingBucket[] = buckets.map(bucket => {
      ...
      const wonLeads = leadsInBucket.filter(lead => lead.status === LeadStatus.WON);
      const lostLeads = leadsInBucket.filter(lead => lead.status === LeadStatus.LOST);
      const activeLeads = leadsInBucket.filter(lead =>
        [LeadStatus.NEW, LeadStatus.QUALIFIED, LeadStatus.PROPOSAL, LeadStatus.NEGOTIATION].includes(lead.status)
      );

      const totalValue = leadsInBucket.reduce((sum, lead) => sum + (lead.estimatedBudget || 0), 0);
      const averageValue = leadsInBucket.length > 0 ? totalValue / leadsInBucket.length : 0;
      const conversionRate = leadsInBucket.length > 0 ? (wonLeads.length / leadsInBucket.length) * 100 : 0;

      return {
        bucket: bucket.name,
        ageRange: bucket.max === Infinity ? `${bucket.min}+ days` : `${bucket.min}-${bucket.max} days`,
  ...
  async getStageTimeAnalysis(startDate?: Date, endDate?: Date): Promise<StageTimeAnalysis[]> {
    let query = this.leadsRepository.createQueryBuilder('lead');
    ...
    for (const stage of stages) {
        ...
        switch (stage) {
          case LeadStatus.NEW:
            if (lead.qualifiedDate) {
              timeInStage = Math.floor(
                (new Date(lead.qualifiedDate).getTime() - new Date(lead.createdDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            } else if (lead.status === LeadStatus.NEW) {
              // Still in NEW stage
              timeInStage = Math.floor(
                (new Date().getTime() - new Date(lead.createdDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
          ...
          case LeadStatus.QUALIFIED:
            if (lead.qualifiedDate && lead.proposalDate) {
              timeInStage = Math.floor(
                (new Date(lead.proposalDate).getTime() - new Date(lead.qualifiedDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            } else if (lead.status === LeadStatus.QUALIFIED && lead.qualifiedDate) {
              timeInStage = Math.floor(
                (new Date().getTime() - new Date(lead.qualifiedDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
          ...
          case LeadStatus.PROPOSAL:
            if (lead.proposalDate && lead.negotiationDate) {
              timeInStage = Math.floor(
                (new Date(lead.negotiationDate).getTime() - new Date(lead.proposalDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            } else if (lead.status === LeadStatus.PROPOSAL && lead.proposalDate) {
              timeInStage = Math.floor(
                (new Date().getTime() - new Date(lead.proposalDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
          ...
          case LeadStatus.NEGOTIATION:
            if (lead.negotiationDate && (lead.wonDate || lead.lostDate)) {
              ...
              timeInStage = Math.floor(
                (new Date(endDate).getTime() - new Date(lead.negotiationDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            } else if (lead.status === LeadStatus.NEGOTIATION && lead.negotiationDate) {
              timeInStage = Math.floor(
                (new Date().getTime() - new Date(lead.negotiationDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
          ...
          case LeadStatus.WON:
            if (lead.wonDate && lead.negotiationDate) {
              timeInStage = Math.floor(
                (new Date(lead.wonDate).getTime() - new Date(lead.negotiationDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
          ...
          case LeadStatus.LOST:
            if (lead.lostDate && lead.negotiationDate) {
              timeInStage = Math.floor(
                (new Date(lead.lostDate).getTime() - new Date(lead.negotiationDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
          ...
          case LeadStatus.DORMANT:
            if (lead.dormantDate) {
              timeInStage = Math.floor(
                (new Date().getTime() - new Date(lead.dormantDate).getTime()) / (1000 * 60 * 60 * 24)
              );
            }
            break;
        }
      ...
      const averageTime = timeData.length > 0 ? timeData.reduce((a, b) => a + b, 0) / timeData.length : 0;
  ...
  async getAgreementCycleTime(startDate?: Date, endDate?: Date): Promise<AgreementCycleTime> {
    let query = this.agreementsRepository.createQueryBuilder('agreement');
    ...
    const medianCycleTime = sortedCycleTimes.length > 0
      ? sortedCycleTimes.length % 2 === 0
        ? (sortedCycleTimes[sortedCycleTimes.length / 2 - 1] + sortedCycleTimes[sortedCycleTimes.length / 2]) / 2
        : sortedCycleTimes[Math.floor(sortedCycleTimes.length / 2)]
      : 0;
    const minCycleTime = sortedCycleTimes.length > 0 ? sortedCycleTimes[0] : 0;
    const maxCycleTime = sortedCycleTimes.length > 0 ? sortedCycleTimes[sortedCycleTimes.length - 1] : 0;

    // Get stage breakdown from agreement stage history
    const stageBreakdown: { stage: string; averageTimeDays: number; percentage: number }[] = [];
    ...
    if (agreementIds.length > 0) {
      ...
      for (const [stage, times] of stageTimesByStage.entries()) {
        ...
        const percentage = totalCycleTime > 0 ? (avgTime / averageCycleTime) * 100 : 0;
  ...
  async getSLAPerformanceByDepartment(startDate?: Date, endDate?: Date): Promise<SLAPerformanceByDepartment[]> {
    const departments = [
      { name: 'Sales', roles: [UserRole.ACCOUNT_MANAGER, UserRole.SALES_MANAGER] },
    ...
    for (const dept of departments) {
      ...
      let notificationsQuery = this.notificationsRepository
        .createQueryBuilder('notification')
        .leftJoinAndSelect('notification.recipient', 'user')
        .where('user.role IN (:...roles)', { roles: dept.roles });

      if (startDate && endDate) {
        notificationsQuery.andWhere('notification.createdDate BETWEEN :startDate AND :endDate', {
          startDate,
          endDate,
        });
      }
      ...
      for (const lead of leads) {
        ...
        const slaConfig = slaConfigs.find(config => {
          if (lead.status === LeadStatus.NEW) return config.stage === SLAStageType.LEAD_NEW;
          if (lead.status === LeadStatus.QUALIFIED) return config.stage === SLAStageType.LEAD_QUALIFIED;
          if (lead.status === LeadStatus.PROPOSAL) return config.stage === SLAStageType.LEAD_PROPOSAL;
          if (lead.status === LeadStatus.NEGOTIATION) return config.stage === SLAStageType.LEAD_NEGOTIATION;
          return false;
        });
        ...
        if (lead.status === LeadStatus.WON || lead.status === LeadStatus.LOST) {
          ...
          const completionTime = lead.closedDate
            ? Math.floor(
                (new Date(lead.closedDate).getTime() - new Date(lead.createdDate).getTime()) / (1000 * 60 * 60 * 24)
              )
            : 0;
      ...
      const complianceRate = totalTasks > 0 ? (completedOnTime / totalTasks) * 100 : 0;
  ...
  async getValueComparison(startDate?: Date, endDate?: Date): Promise<ValueComparison> {
    let leadsQuery = this.leadsRepository.createQueryBuilder('lead');
    ...
    const pipelineValue = pipelineLeads.reduce((sum, lead) => sum + (lead.estimatedBudget || 0), 0);
    const pipelineCount = pipelineLeads.length;

    // Calculate ratio
    const ratio = pipelineValue > 0 ? signedValue / pipelineValue : 0;
    const conversionRate = leads.length > 0 ? (signedCount / leads.length) * 100 : 0;

    // Breakdown by stage
    const byStage: { stage: string; count: number; value: number; percentage: number }[] = [];

    const stages = [LeadStatus.NEW, LeadStatus.QUALIFIED, LeadStatus.PROPOSAL, LeadStatus.NEGOTIATION];

    for (const stage of stages) {
      const stageLeads = leads.filter(lead => lead.status === stage);
      const stageValue = stageLeads.reduce((sum, lead) => sum + (lead.estimatedBudget || 0), 0);
