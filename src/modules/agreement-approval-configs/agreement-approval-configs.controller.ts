...
export class AgreementApprovalConfigsController {
  ...
  create(@Body() createDto: CreateAgreementApprovalConfigDto, @Request() req) {
    return this.agreementApprovalConfigsService.create(
      createDto,
      req.user.userId,
    );
  }
  ...
  @Roles(UserRole.ADMIN, UserRole.LEGAL)
  defineApprovalFlow(
    @Body() dto: DefineAgreementApprovalFlowDto,
    @Request() req,
  ) {
    return this.agreementApprovalConfigsService.defineApprovalFlow(
      dto,
      req.user.userId,
    );
  }
