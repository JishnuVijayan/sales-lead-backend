...
export class PdfService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    ...
    if (process.env.DISABLE_PUPPETEER_ON_STARTUP === 'true') {
      this.logger.log(
        'Puppeteer startup disabled by DISABLE_PUPPETEER_ON_STARTUP',
      );
      return;
    }
    ...
    try {
      ...
      this.browser = null;
      this.logger.warn(
        `Puppeteer failed to launch on startup: ${err?.message || err}`,
      );
    }
  }
  ...
  async generateProposalPdf(proposal: Proposal): Promise<Buffer> {
    ...
    if (!this.browser) {
      try {
        ...
        this.logger.log(
          'Puppeteer browser lazily initialized for PDF generation',
        );
      } catch (err: any) {
        this.logger.error(
          `Unable to initialize Puppeteer for PDF generation: ${err?.message || err}`,
        );
        throw new Error(
          'PDF generation is currently unavailable in this environment.',
        );
      }
    }
    ...
    const templateData = {
      ...
      discountAmount: parseFloat(
        proposal.discountAmount?.toString() || '0',
      ).toFixed(2),
      totalAmount: parseFloat(proposal.totalAmount?.toString() || '0').toFixed(
        2,
      ),
      taxPercent: parseFloat(proposal.taxPercent?.toString() || '0'),
      discountPercent: parseFloat(proposal.discountPercent?.toString() || '0'),
      termsAndConditions:
        proposal.termsAndConditions || 'Standard terms and conditions apply.',
    };
